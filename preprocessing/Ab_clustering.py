...
def list_AbAg(tsv_path, merged_fasta_path, save_path):
    ...
    antigen_type = ['protein' in antigen.lower() or 'peptide' in antigen.lower() for antigen in antigen_type]
    protein_antigen_lines = [line for line, is_protein in zip(with_antigen_lines, antigen_type) if is_protein]
    ...
    def _get_ab_data(line):
        ...
        pair = [(label, auth) for group, label, auth in zip(group_pdb, label_asym_id, auth_asym_id) if group == 'ATOM']
        pair = list(set(pair))
    ...
    Ab_chain_ids = Parallel(n_jobs=-1)(delayed(_get_ab_data)(line) for line in protein_antigen_lines)
