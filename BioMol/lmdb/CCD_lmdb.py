...
def parse_cif(cif_path):
    ...
    output_1D["full_atoms"] = Feature1D(
        "full_atoms", full_atoms, full_atoms_mask, feature_level, None
    )
    ...
    ideal_coords = torch.stack(
        [
            output_1D["ideal_x"].feature()[0],
            output_1D["ideal_y"].feature()[0],
            output_1D["ideal_z"].feature()[0],
        ]
    ).T  # (N, 3) or (3,)
    if ideal_coords.dim() == 1:
        ideal_coords = ideal_coords.unsqueeze(0)
    ideal_coords_mask = torch.stack(
        [
            output_1D["ideal_x"].feature()[1],
            output_1D["ideal_y"].feature()[1],
            output_1D["ideal_z"].feature()[1],
        ]
    ).T  # (N, 3)
    ideal_coords_mask = torch.all(ideal_coords_mask, dim=1)  # (N,)
    ideal_coords = Feature1D(
        "ideal_coords", ideal_coords, ideal_coords_mask, feature_level, None
    )
    ...
    if "_chem_comp_bond.atom_id_1" in mmcif_dict:
        ...
        for bond_idx in range(bond_number):
            ...
            if (
                atom1 not in output_1D["full_atoms"].value
                or atom2 not in output_1D["full_atoms"].value
            ):
                continue
