...
from BioMol.utils.hierarchy import (
    ChemComp,
    ChemCompView,
    Polymer,
    NonPolymer,
    ...
    PolymerType,
    AsymmetricChain,
    AsymmetricChainStructure,
    BioAssembly,
)
from BioMol.utils.parser_utils import read_lmdb_value, get_smaller_mmcif_dict, merge_dict
from BioMol.constant.chemical import stereo_config_map, num2residue, num2AA
from BioMol import IDEAL_LIGAND_PATH
...
def parse_chem_comp(chem_comp_id, chem_comp_item):
    ...
    for key in config_1D:
        ...
        if key_in_mmcif_dict not in chem_comp_item:
            continue
        data = chem_comp_item[key_in_mmcif_dict]
        data = [data] if data is str else data
        ...
        if data_type is not str:
            output_1D[key] = Feature1D(
                key, torch.tensor(data), torch.tensor(mask), feature_level, None
            )
        else:
    ...
    if "_chem_comp_bond.atom_id_1" in chem_comp_item:
        ...
        _chem_comp_bond_aromatic_flag = chem_comp_item[
            "_chem_comp_bond.pdbx_aromatic_flag"
        ]
        _chem_comp_bond_stereo_config = chem_comp_item[
        ...
        for bond_idx in range(bond_number):
            ...
            stereo_config = stereo_config_map.get(
                _chem_comp_bond_stereo_config[bond_idx], -1
            )
        ...
        output_2D["bond_type"] = FeaturePair(
            "bond_type", bond_type_dict, feature_level, True, None
        )
        output_2D["aromatic"] = FeaturePair(
            "aromatic", aromatic_flag_dict, feature_level, True, None
        )
        output_2D["stereo"] = FeaturePair(
            "stereo", stereo_config_dict, feature_level, True, None
        )

    else:
        output_2D = {}

    output = {"0D": output_0D, "1D": output_1D, "2D": output_2D}
...
def compare_ideal_chem_comp(chem_comp):
    ...
    for key in key_1d:
        if key in chem_comp["1D"]:
            ...
            if type(in_item) is list:
                if in_item != ideal:
                    if len(in_item) != len(ideal):
                        chem_comp_help[key] = (
                            f"{key} length between loaded item and external source is \
                                different. You should check it."
                        )
                    else:
                        diff = [
                            idx
                            for idx, (x, y) in enumerate(zip(in_item, ideal))
                            if x != y
                        ]
                        diff = ",".join(diff)
                        chem_comp_help[key] = f"{key} is different at {diff}"
            elif type(in_item) is torch.Tensor:
                if torch.any(in_item != ideal):
                    if in_item.shape != ideal.shape:
                        chem_comp_help[key] = (
                            f"{key} shape between loaded item and external source is \
                                different. You should check it."
                        )
                    else:
            ...
            chem_comp["1D"][key] = Feature1D(
                key, ideal_value, ideal_mask, FeatureLevel.CHEMCOMP, additional_info
            )
...
def parse_struct_ref_dict(struct_ref_dict):
    ...
    output_dict = {}
    for value in struct_ref_dict.values():
        key_list = list(value.keys())
        entity_id = value["_struct_ref.entity_id"]
        if entity_id not in output_dict:
            output_dict[entity_id] = {
                key: value[key] for key in key_list if key != "_struct_ref.entity_id"
            }
        else:
            for key in output_dict[entity_id].keys():
                if type(output_dict[entity_id][key]) is str:
                    output_dict[entity_id][key] = [output_dict[entity_id][key]] + value[
                        key
                    ]
                elif type(output_dict[entity_id][key]) is list:
                    output_dict[entity_id][key] = (
                        output_dict[entity_id][key] + value[key]
                    )
...
def parse_polymer_entity(items, chem_comp_dict):
    ...
    # parse sequence
    polymer_0D["polymer_type"] = Feature0D(
        "polymer_type", PolymerType(items["_entity_poly.type"]), feature_level, None
    )  # protein : polypeptide(L)
    polymer_0D["auth_asym_id"] = Feature0D(
        "auth_asym_id", items["_entity_poly.pdbx_strand_id"], feature_level, None
    )
    ...
    polymer_1D["one_letter_code"] = Feature1D(
        "one_letter_code", seq_one_letter_code, None, feature_level, None
    )
    polymer_1D["one_letter_code_can"] = Feature1D(
        "one_letter_code_can", seq_one_letter_code_can, None, feature_level, None
    )
    ...
    polymer_1D["chem_comp_list"] = Feature1D(
        "chem_comp_list", chem_comp_list, None, feature_level, None
    )
    polymer_1D["chem_comp_hetero"] = Feature1D(
        "chem_comp_hetero", hetero_list, None, feature_level, None
    )
...
def parse_nonpolymer_entity(items, chem_comp_dict, is_water=False):
    ...
    nonpolymer_0D = {}
    feature_level = FeatureLevel.ENTITY
    nonpolymer_0D["name"] = Feature0D(
        "name",
        items.get("_pdbx_entity_nonpoly.entity_nonpoly", None),
        feature_level,
        None,
    )
    nonpolymer_0D["chem_comp"] = Feature0D(
        "chem_comp", items.get("_pdbx_entity_nonpoly.comp_id", None), feature_level, None
    )
...
def parse_branched_entity(items, chem_comp_dict):
    ...
    branched_0D = {
        "descriptor": Feature0D(
            "descriptor",
            items.get("_pdbx_entity_branch_descriptor.descriptor", None),
            feature_level,
            None,
        ),
        "type": Feature0D(
            "type",
            items.get("_pdbx_entity_branch_descriptor.type", None),
            feature_level,
            None,
        ),
        "program": Feature0D(
            "program",
            items.get("_pdbx_entity_branch_descriptor.program", None),
            feature_level,
            None,
        ),
        "program_version": Feature0D(
            "program_version",
            items.get("_pdbx_entity_branch_descriptor.program_version", None),
            feature_level,
            None,
        ),
    }
    ...
    branched_1D = {
        "chem_comp_list": Feature1D(
            "chem_comp_list", chem_comp_list, None, feature_level, None
        ),
        "chem_comp_num": Feature1D(
            "chem_comp_num", seq_num_list, None, feature_level, None
        ),
        "chem_comp_hetero": Feature1D(
            "chem_comp_hetero", hetero_list, None, feature_level, None
        ),
    }
    ...
    leaving_atom_id_1_list = items.get(
        "_pdbx_entity_branch_link.leaving_atom_id_1", None
    )
    leaving_atom_id_2_list = items.get(
        "_pdbx_entity_branch_link.leaving_atom_id_2", None
    )
    bond_list = items.get("_pdbx_entity_branch_link.value_order", None)
    entity_branch_list_num_1_list = items.get(
        "_pdbx_entity_branch_link.entity_branch_list_num_1", None
    )
    entity_branch_list_num_2_list = items.get(
        "_pdbx_entity_branch_link.entity_branch_list_num_2", None
    )

    if type(comp_id_1_list) is str:
    ...
    for idx in range(len(comp_id_1_list)):
        ...
        branch_link[(entity_branch_list_num_1, entity_branch_list_num_2)] = {
            "atom_id": (atom_id_1, atom_id_2),
...
def parse_str_dict(str_dict, _struct_site_dict):
    ...
    for asym_id, items in str_dict.items():
        ...
        idx_dict["cif_idx_list"] = to_int_tensor(
            items["_atom_site.label_seq_id"]
        )  # it can be empty for ligand or water.
        ...
        x, y, z = (
            items["_atom_site.Cartn_x"],
            items["_atom_site.Cartn_y"],
            items["_atom_site.Cartn_z"],
        )
...
def parse_poly_scheme_dict(poly_scheme_dict):
    ...
    auth_idx_list = [
        aa if ii == "." else f"{aa}.{ii}" for aa, ii in zip(auth_idx, ins_code)
    ]
    ...
    return Scheme(
        entity_id, scheme_type, cif_idx_list, auth_idx_list, chem_comp_list, hetero_list
    )
...
def parse_nonpoly_scheme_dict(nonpoly_scheme_dict):
    ...
    cif_idx = list(range(len(auth_idx)))
...
def parse_branched_scheme_dict(branched_scheme_dict):
    ...
    cif_idx_list = list(range(len(auth_idx_list)))
...
def remove_UNL_from_atom_site(_atom_site_dict):
    ...
    for asym_id in _atom_site_dict.keys():
        ...
        for key in _atom_site_dict[asym_id].keys():
            new_dict[asym_id][key] = [
                _atom_site_dict[asym_id][key][idx]
                for idx in range(len(UNL_mask))
                if UNL_mask[idx]
            ]
...
def remove_UNL_from_struct_conn(_struct_conn_dict):
    ...
    for asym_pair in _struct_conn_dict.keys():
        label_comp_id1_list = _struct_conn_dict[asym_pair][
            "_struct_conn.ptnr1_label_comp_id"
        ]
        label_comp_id2_list = _struct_conn_dict[asym_pair][
            "_struct_conn.ptnr2_label_comp_id"
        ]
        UNL_mask = [
            cc1 != "UNL" and cc2 != "UNL"
            for cc1, cc2 in zip(label_comp_id1_list, label_comp_id2_list)
        ]
        if not any(UNL_mask):
            del new_dict[asym_pair]
            continue
        for key in _struct_conn_dict[asym_pair].keys():
            new_dict[asym_pair][key] = [
                new_dict[asym_pair][key][idx]
                for idx in range(len(UNL_mask))
                if UNL_mask[idx]
            ]
...
def get_pdbx_struct_oper(pdbx_struct_oper_dict):
    ...
    for struct_oper_id in pdbx_struct_oper_dict.keys():
        ...
        output_dict[struct_oper_id]["symmetry_operation"] = raw_dict[
...
def parse_cif(cif_path, cif_configs=None, remove_signal_peptide=False):
    ...
    deposition_date = mmcif_dict["_pdbx_database_status.recvd_initial_deposition_date"][
        0
    ]
    ...
    _chem_comp_atom_dict = get_smaller_mmcif_dict(
        mmcif_dict, mmcif_dict_keys, "_chem_comp_atom"
    )
    _chem_comp_bond_dict = get_smaller_mmcif_dict(
        mmcif_dict, mmcif_dict_keys, "_chem_comp_bond"
    )
    ...
    _pdbx_poly_seq_scheme_dict = get_smaller_mmcif_dict(
        mmcif_dict, mmcif_dict_keys, "_pdbx_poly_seq_scheme", "asym_id"
    )
    _pdbx_nonpoly_scheme_dict = get_smaller_mmcif_dict(
        mmcif_dict, mmcif_dict_keys, "_pdbx_nonpoly_scheme", "asym_id"
    )
    _pdbx_branch_scheme_dict = get_smaller_mmcif_dict(
        mmcif_dict, mmcif_dict_keys, "_pdbx_branch_scheme", "asym_id"
    )
    ...
    # by asym_id
    _atom_site_dict = get_smaller_mmcif_dict(
        mmcif_dict, mmcif_dict_keys, "_atom_site", "label_asym_id"
    )
    ...
    # get entity seq info
    _entity_dict = get_smaller_mmcif_dict(
        mmcif_dict, mmcif_dict_keys, "_entity", "id", len1_to_scalar=True
    )
    _entity_poly_dict = get_smaller_mmcif_dict(
        mmcif_dict, mmcif_dict_keys, "_entity_poly", "entity_id", len1_to_scalar=True
    )
    _entity_poly_seq_dict = get_smaller_mmcif_dict(
        mmcif_dict,
        mmcif_dict_keys,
        "_entity_poly_seq",
        "entity_id",
        len1_to_scalar=False,
    )
    _pdbx_entity_nonpoly_dict = get_smaller_mmcif_dict(
        mmcif_dict,
        mmcif_dict_keys,
        "_pdbx_entity_nonpoly",
        "entity_id",
    ...
    _pdbx_entity_branch_descriptor_dict = get_smaller_mmcif_dict(
        mmcif_dict,
        mmcif_dict_keys,
    ...
    _pdbx_entity_branch_link_dict = get_smaller_mmcif_dict(
        mmcif_dict,
        mmcif_dict_keys,
    ...
    _pdbx_entity_branch_list_dict = get_smaller_mmcif_dict(
        mmcif_dict,
        mmcif_dict_keys,
    ...
    _struct_conn_dict = get_smaller_mmcif_dict(
        mmcif_dict,
        mmcif_dict_keys,
        "_struct_conn",
        "ptnr1_label_asym_id",
    ...
    _pdbx_struct_oper_dict = get_smaller_mmcif_dict(
        mmcif_dict, mmcif_dict_keys, "_pdbx_struct_oper_list", "id", len1_to_scalar=True
    )
    ...
    # ChemComp idx
    chem_comp_idx_map = {
        chem_copm: ideal_ligand_list.index(chem_copm)
        for chem_copm in chem_comp_dict.keys()
    }
...
def parse_simple_pdb(pdb_path, cif_configs=None):
    ...
    one_letter_code = [
        num2AA[num2residue.index(residue)] for residue in chem_comp_str_list
    ]
    ...
    # parse sequence
    polymer_0D["polymer_type"] = Feature0D(
        "polymer_type", PolymerType("polypeptide(L)"), feature_level, None
    )  # protein : polypeptide(L)
    ...
    scheme = Scheme(
        "1",
        scheme_type,
        [ii + 1 for ii in range(seq_num)],
        [str(ii + 1) for ii in range(seq_num)],
        chem_comp_str_list,
        hetero_list,
    )
    ...
    _atom_site_dict, remove_UNL = remove_UNL_from_atom_site(_atom_site_dict)
    _atom_site_dict["A"]["_atom_site.label_entity_id"] = [
        "1" for _ in range(len(_atom_site_dict["A"]["_atom_site.label_entity_id"]))
    ]
