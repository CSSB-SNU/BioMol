...
def get_frequency(graph_dir,node_save_path,edge_save_path,graph_save_path):
    ...
    for k, v in graph_dict.items():
        ...
        if graph_hash not in graph_freq:
            ...
            graph_freq[graph_hash] = (graph_freq[graph_hash][0] + 1 / graph_num, v, graph_freq[graph_hash][2] + [k])
...
def get_most_frequent_item(node_pickle_path, edge_pickle_path, graph_pickle_path, chain_ID_to_cluster_path, top_k):
    with open(node_pickle_path, 'rb') as f:
        node_freq = pickle.load(f)
    with open(edge_pickle_path, 'rb') as f:
        edge_freq = pickle.load(f)
    ...
    for rank, edge in enumerate(frequent_edges):
        ...
        frequent_edge_chain_IDs[rank] = [(src_chain, tgt_chain) for src_chain, tgt_chain in zip(src_chain_IDs, tgt_chain_IDs)]
...
def unittest_level2_Cluster(level0_save_path2, level2_cluster_path):
    ...
    if not os.path.exists(connectivity_matrix_path):
        ...
        pairs = [(i, j, graph_list[i], graph_list[j]) for i, j in combinations(range(n_cluster), 2)]
        ...
        results = Parallel(n_jobs=cpu_num)(
            delayed(compute_connection)(i, j, graph1,graph2) for i, j, graph1, graph2 in pairs
        )
...
def calculate_graph_diameter(adj_matrix):
    ...
    max_distances = Parallel(n_jobs=-1)(
        delayed(bfs_max_distance)(i) for i in range(n)
    )
...
if __name__ == '__main__':
    ...
    chain_ID_to_cluster_path = "/data/psk6950/PDB_2024Mar18/protein_seq_clust/v2_chainID_to_cluster.pkl"
